...
function buildFallbackImagePrompt(brief) {
    ...
    return [
        `Harsh-flash tabloid reportage cover about ${company}.`,
        ...
        'Use a vivid physical scene with visible conflict, embarrassment, public reaction, oversized symbolic props, and messy viral-photo energy.',
        '3:4 portrait composition, realistic phone or paparazzi camera perspective, harsh flash or security-camera feel, leave lower area usable for TemplateV1 cover text.',
...
function normalizeConcept(concept, article) {
    ...
    const normalized = {
        name: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.name, article), article), 120),
        visual_style: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.visual_style, article), article), 180),
        scene_context: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.scene_context, article), article), 220),
        satirical_action: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.satirical_action, article), article), 260),
        why_location_fits: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.why_location_fits, article), article), 260),
        why_it_works: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.why_it_works, article), article), 260),
        risk: compact(sanitizeObviousMetaphorText(sanitizeNoReadableTextInstruction(concept.risk, article), article), 220),
...
function hasHeadlineQualityRisk(value) {
    ...
    if (/КРАСН\p{L}*\s+ЛЕНТ|ФЕЙСКОНТРОЛ|НЕ\s+ПУСКАЕТ\s+ИИ\s+ЗА/iu.test(headline)) {
        return true;
    }
    if (/(АЙТИШНИК|ПРОВАЛИТЬ\s+ТЕСТ|ДОСТУП\s+К\s+ИСПЫТАНИЯМ|СИМУЛЯЦИОНН\p{L}*\s+КАМЕР|ИСПЫТАТЕЛЬН\p{L}*\s+КАМЕР|ЗАПЕР\p{L}*\s+SAM\s+ALTMAN|ПРОВЕРЯЕТ\s+ИИ\s+ДО\s+РЕЛИЗА|ОТКРЫТИЯ\s+OPENAI\s+ПОД\s+ДАВЛЕНИЕМ)/iu.test(headline)) {
        return true;
    }
    if (/\bDEPLOYMENT\s+SIMULATION\b|КОНКУРЕНТ\p{L}*\s+В\s+КЛЕТК/iu.test(headline)) {
        return true;
    }
...
function hasStaticDemoSceneRisk(planText) {
    ...
    if (hasSimulationEscapeCue(text)) return false;
    ...
    const strongSatireCue = /\b(embarrass(?:ed|ment|ing)?|humiliat(?:ed|ing|ion)|status reversal|panic(?:s|king)?|public shame|caught on camera|leaked flash|paparazzi flash|denied badge|blocked turnstile|bouncer|velvet rope|security guard|confiscated|dragged|sweating|tiny chair|remote control|auction paddle|stampede|office mutiny|competitors? watching helplessly|public line|forced into|locked out|thrown out|held back|squeezed|crushed by paperwork)\b|паник|отчаян|унижен|позор|блокир|заблокир|не\s+пуска|сдержив|турникет|красн\w*\s+лент|бархатн\w*\s+вер[её]в|охран|очеред|пыта[ею]тся\s+(?:достать|войти|пролезть|сорвать|обойти)/iu
        .test(text);

    return !strongSatireCue;
}

function isDeploymentSimulationArticle(article) {
    const articleText = String([
        article?.raw_title || article?.title || '',
        article?.raw_summary || article?.summary || '',
        article?.raw_text || ''
    ].join(' '));
    return /\b(?:deployment simulation|simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(articleText);
}

function hasSimulationEscapeCue(text) {
    const value = String(text || '');
    const virtualCue = /\b(?:matrix[-\s]+like|matrix|virtual simulation|digital simulation|simulation grid|simulated program|inside the program|green code|digital cage|virtual reality)\b/i
        .test(value);
    const escapeCue = /\b(?:break(?:s|ing)? out|tear(?:s|ing)? out|rip(?:s|ping)? out|escape(?:s|ing)?|pull(?:s|ing)? free|trying to leave|trying to get out|keeps? (?:it|the model) inside|hold(?:s|ing)? (?:it|the model) inside)\b/i
        .test(value);
    const modelCue = /\b(?:AI avatar|AI model|model avatar|symbolic robot|robot|humanoid)\b/i.test(value);
    const controllerCue = /\b(?:Sam Altman|founder|engineer|operator|controller|computer|terminal|keyboard|laptop)\b/i.test(value);
    return virtualCue && escapeCue && modelCue && controllerCue;
}

function allowsSimulationEscapeAvatar(article, text) {
    return isDeploymentSimulationArticle(article) && hasSimulationEscapeCue(text);
}

function hasBannedDeploymentSimulationSceneRisk(article, planText) {
    if (!isDeploymentSimulationArticle(article)) {
        return false;
    }
    return /\b(?:simulation chamber|test chamber|transparent chamber|high[-\s]+tech test chamber|quarantine booth|glass chamber|trial chamber|transparent box|sealed chamber)\b|испытательн\p{L}*\s+камер|симуляционн\p{L}*\s+камер/iu
        .test(String(planText || ''));
}

function hasUnclearDeploymentSimulationRisk(article, planText) {
    if (!isDeploymentSimulationArticle(article)) {
        return false;
    }
    const text = String(planText || '');
    if (hasSimulationEscapeCue(text)) return false;
    const clearTestCue = /\b(crash[-\s]+test|stress[-\s]+test|safety inspection|pre[-\s]+release inspection|mock deployment|trial run|test track|obstacle course|emergency drill|failure drill|quality control|inspection table|red[-\s]+team drill)\b|краш[-\s]?тест|стресс[-\s]?тест|тестов\p{L}*\s+полигон|до\s+релиза|предрелизн\p{L}*\s+провер|провер\p{L}*\s+до\s+релиза|обкат\p{L}*/iu
        .test(text);
    const unclearAccessCue = /\b(bouncer|velvet rope|red rope|access card|blocked gate|strict bouncer|launch-party bouncer)\b|красн\p{L}*\s+лент|бархатн\p{L}*\s+вер[её]в|фейсконтрол|пропуск\p{L}*\s+пункт/iu
        .test(text);
    return unclearAccessCue && !clearTestCue;
}

function hasFounderDominatesDeploymentRisk(article, planText) {
    const articleText = String([
        article?.raw_title || article?.title || '',
        article?.raw_summary || article?.summary || '',
        article?.raw_text || ''
    ].join(' '));
    if (!/\b(?:simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(articleText)) {
        return false;
    }
    const text = String(planText || '');
    if (allowsSimulationEscapeAvatar(article, text)) return false;
    const founderForeground = /\b(?:Sam Altman|Dario Amodei|Sundar Pichai|Mark Zuckerberg|Elon Musk|Tim Cook|Jensen Huang)\b[^.!?]{0,90}\b(?:foreground|center|central|close-up|portrait|holding a remote|holding a clipboard)\b|\b(?:foreground|center|central|close-up|portrait)\b[^.!?]{0,90}\b(?:Sam Altman|Dario Amodei|Sundar Pichai|Mark Zuckerberg|Elon Musk|Tim Cook|Jensen Huang)\b/i
        .test(text);
    const modelDominates = /\b(?:central|dominant|foreground|main)\b[^.!?]{0,140}\b(?:black[-\s]+box|server unit|model unit|crash[-\s]+test sled|test rig)\b|\b(?:black[-\s]+box|server unit|model unit|crash[-\s]+test sled|test rig)\b[^.!?]{0,140}\b(?:central|dominant|foreground|main)\b/i
        .test(text);
    const modelPushedBehind = /\b(?:black[-\s]+box|server unit|model unit|crash[-\s]+test sled|test rig)\b[^.!?]{0,140}\b(?:behind him|behind her|in the background|somewhere)\b|\b(?:behind him|behind her|in the background|somewhere)\b[^.!?]{0,140}\b(?:black[-\s]+box|server unit|model unit|crash[-\s]+test sled|test rig)\b/i
        .test(text);
    return (founderForeground && !modelDominates) || modelPushedBehind;
}
...
function hasWeakRagebaitVisualRisk(planText) {
    const text = String(planText || '');
    const polishedCue = /\b(premium|luxurious|dramatic but natural light|natural lighting|clean lower[-\s]+third|clean negative space|modern tech conference|high[-\s]+tech server room|futuristic runway|editorial magazine cover|cinematic lighting|polished editorial|beautiful studio|calm executive portrait)\b/i
        .test(text);
    if (polishedCue) return true;

    const cameraCue = /\b(leaked|leak|harsh[-\s]+flash|flash photo|paparazzi|tabloid|phone reportage|phone photo|phone camera|phone snapshot|candid office photo|low[-\s]+quality candid|security camera|cctv|surveillance|viral screenshot|press scrum|reporter scrum|caught on camera|backstage photo|messy viral)\b/i
        .test(text);
    const conflictCue = /\b(embarrass(?:ed|ment|ing)?|humiliat(?:ed|ing|ion)|panic(?:s|king)?|public shame|blocked|denied|locked out|held back|bouncer|security guard|velvet rope|confiscated|dragged|sweating|tiny chair|stampede|office mutiny|competitors? watching helplessly|public line|forced into|thrown out|squeezed|crushed|interrogation|raid|meltdown|scramble|shouting|pleading|surrounded|status reversal|caught on camera)\b|паник|унижен|позор|блокир|заблокир|не\s+пуска|сдержив|турникет|охран|очеред|стыд|толпа|давк|рейд|обыск|зажат|пойман/iu
        .test(text);

    return !(cameraCue && conflictCue);
}
...
function hasUnsupportedCompetitorRisk(article, planText) {
    const articleText = String([
        article?.raw_title || article?.title || '',
        article?.raw_summary || article?.summary || '',
        article?.raw_text || ''
    ].join(' '));
    const isInternalDeploymentStory = /\b(?:simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(articleText);
    if (!isInternalDeploymentStory) return false;
    const plan = String(planText || '');
    const planUsesCompetitors = /\b(?:competitors?|rivals?)\b|конкурент/iu.test(plan);
    const articleUsesCompetitors = /\b(?:competitors?|rivals?|against|versus|vs\.?)\b|конкурент/iu.test(articleText);
    return planUsesCompetitors && !articleUsesCompetitors;
}
...
function sanitizeObviousMetaphorText(value, article) {
    ...
    if (!allowsSimulationEscapeAvatar(article, text)) {
        text = text
...
function buildObviousMetaphorBans(article) {
    ...
    if (/\b(?:simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(text)) {
        bans.push('Deployment simulation / pre-release behavior stories: hard-ban generic simulation chambers, quarantine booths, transparent boxes, transparent cages, generic bouncers, red/velvet ropes, access-card gatekeeping, and founder portraits as the main joke. Prefer a Matrix-like virtual-simulation escape metaphor when it fits: a symbolic AI avatar/model tries to tear out of a glowing simulated program while Sam Altman or engineers panic at a computer trying to keep it inside before release. Alternative: a visible crash-test / stress-test / safety inspection. Do not copy exact movie characters, readable code, interface text, generated logos, or brand marks.');
    }
...
function sanitizeNoReadableTextInstruction(value, article = null) {
    const text = String(value || '')
    ...
    return cleanNegativeReplacementArtifacts(sanitizeRobotTerms(text, article));
}

function sanitizeRobotTerms(value, article = null) {
    let text = String(value || '');
    if (allowsSimulationEscapeAvatar(article, text)) return text;
    return text
...
function sanitizeImagePrompt(value, brief, article = null) {
    let text = sanitizeNoReadableTextInstruction(restoreProtectedTerms(value, brief), article);
    ...
    text = text
        ...
        .replace(/“[^”]{1,90}”/g, 'blank')
        .replace(/\bcartoonish\b/giu, 'realistic satirical');
    text = sanitizeRobotTerms(text, article);
...
Return ONLY valid JSON. Keep this JSON compact; do not repeat fields already present in article_brief:
{
  ...
  "visual": {
    "image_prompt": "English prompt for image model. Harsh-flash tabloid/ragebait reportage cover scene. No readable text/logos/watermarks.",
...
- Target format: viral Russian AI tabloid cover, not a calm tech magazine. The cover should feel like a leaked phone photo, paparazzi flash, press scrum, security-camera screenshot, messy backstage shot, or dirty reportage frame.
- The visual should have conflict and social pressure: someone is blocked, embarrassed, denied, caught by flash, pushed into a test, held behind security, publicly losing status, or watching helplessly. A clean symbolic object by itself is not enough.
- Do not make "premium editorial", "natural lighting", "clean negative space", "futuristic runway", "high-tech server room", or polished founder portraits unless the scene is visibly messy and humiliating.
...
- Headline must communicate the actual article meaning first, then add punch. Do not sacrifice comprehension for a joke.
...
- Normally do not visualize an AI model as a robot, humanoid, mascot, cute figurine, or generic sci-fi character. Exception: for deployment-simulation stories, a symbolic AI avatar/robot may be used only if it is clearly trying to escape a virtual simulation/program while humans try to keep it contained before release.
...
- Ragebait rule: if the image looks like a respectable press-release illustration, it fails. Push it toward a realistic trashy viral news photo: harsh flash, crowd reaction, public awkwardness, power imbalance, and one instantly readable absurd action.
- Deployment-simulation rule: if the article is about predicting model behavior before release, prefer a Matrix-like virtual-simulation escape scene: the model/avatar tries to break out of a simulated program while Sam Altman or engineers panic at computers trying to keep it inside. A crash-test/stress-test/safety-inspection scene is acceptable only if it is more instantly understandable. Do not use generic bouncers, red ropes, access cards, or simple gatekeeping.
...
function normalizeContentPlan(candidate, article, brief, fallback = buildFallbackContentPlan(article, brief)) {
    ...
    const sanitizedImagePrompt = sanitizeObviousMetaphorText(sanitizeImagePrompt(visual.image_prompt || fallback.visual.image_prompt, brief, article), article);
    ...
    const rawDirectorForRisk = parsed.creative_director && typeof parsed.creative_director === 'object'
        ? { ...parsed.creative_director, rejected_obvious_metaphor: '' }
        : {};
    const planTextForObviousCheck = [
        visual.image_prompt,
        JSON.stringify(rawDirectorForRisk),
    ...
    if (!falseGovernmentPressure && hasBannedDeploymentSimulationSceneRisk(article, planTextForObviousCheck) && !creativeDirector.quality_flags.includes('banned_deployment_chamber_risk')) {
        creativeDirector.quality_flags.push('banned_deployment_chamber_risk');
    }
    if (!falseGovernmentPressure && hasUnclearDeploymentSimulationRisk(article, planTextForObviousCheck) && !creativeDirector.quality_flags.includes('unclear_deployment_simulation_risk')) {
        creativeDirector.quality_flags.push('unclear_deployment_simulation_risk');
    }
    if (!falseGovernmentPressure && hasFounderDominatesDeploymentRisk(article, planTextForObviousCheck) && !creativeDirector.quality_flags.includes('founder_dominates_deployment_risk')) {
        creativeDirector.quality_flags.push('founder_dominates_deployment_risk');
    }
    ...
    if (!falseGovernmentPressure && hasWeakRagebaitVisualRisk(planTextForObviousCheck) && !creativeDirector.quality_flags.includes('weak_ragebait_visual_risk')) {
        creativeDirector.quality_flags.push('weak_ragebait_visual_risk');
    }
    ...
    if (!falseGovernmentPressure && hasUnsupportedCompetitorRisk(article, `${planTextForFactCheck} ${planTextForObviousCheck}`)) {
        creativeDirector.quality_flags.push('unsupported_competitor_risk');
    }
