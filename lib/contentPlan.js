...
function buildObviousMetaphorBans(article) {
    ...
    if (/\b(?:simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(text)) {
        bans.push('Deployment simulation / pre-release behavior stories: hard-ban simulation chambers, test chambers, quarantine booths, transparent boxes, transparent cages, and public figures locked inside chambers. Do not make Sam Altman the trapped object. Make the simulation a social power scene: Sam Altman as a harsh launch-party bouncer denying a badge to an overconfident AI team, a stress-test interrogation table where executives sweat under harsh flash, a security checkpoint where competitors are held behind velvet rope, or a public safety drill where everyone panics except the person holding the access key.');
    }
