...
function buildObviousMetaphorBans(article) {
    ...
    if (/\b(?:simulat(?:e|es|ed|ing|ion)|deployment|before release|model behavior before release)\b/i.test(text)) {
        bans.push('Deployment simulation / pre-release behavior stories: hard-ban simulation chambers, test chambers, quarantine booths, transparent boxes, transparent cages, public figures locked inside chambers, generic bouncers, red/velvet ropes, access-card gatekeeping, and founder portraits as the main joke. The viewer must understand "OpenAI crash-tests / stress-tests models before release" in 2 seconds. The central visual subject must be the black-box model/server unit on a crash-test sled, test rig, obstacle course, inspection table, or fake deployment disaster rehearsal. Sam Altman can be visible only as a side inspector/controller, not the main portrait.');
    }
...
- Deployment-simulation rule: if the article is about predicting model behavior before release, the cover must read as "pre-release crash-test / stress-test / safety inspection" in 2 seconds. Do not use generic bouncers, red ropes, access cards, or "not letting AI through a gate" unless a visible crash-test/stress-test setup is also present.
