...
function hasHeadlineQualityRisk(value) {
    ...
    if (/(АЙТИШНИК|ПРОВАЛИТЬ\s+ТЕСТ|ДОСТУП\s+К\s+ИСПЫТАНИЯМ|СИМУЛЯЦИОНН\p{L}*\s+КАМЕР|ПРОВЕРЯЕТ\s+ИИ\s+ДО\s+РЕЛИЗА|ОТКРЫТИЯ\s+OPENAI\s+ПОД\s+ДАВЛЕНИЕМ)/iu.test(headline)) {
        return true;
    }
